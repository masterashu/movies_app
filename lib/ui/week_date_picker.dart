              ...
              for (var i = 0; i < widget.days.length; i++)
                GestureDetector(
                  onTap: () {
                    ...
                    if (widget.onSelected != null) widget.onSelected(widget.dates[i]);
                  },
                  child: AnimatedBuilder(
                    animation: widget.controller,
                    builder: (context, child) {
                      var opacity = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(
                        curve: Interval(0.72, 0.94 + i * 0.01, curve: Curves.easeIn),
                        parent: widget.controller,
                      ));

                      var scale = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(
                        curve: Interval(0.72, 0.94 + i * 0.01, curve: Curves.easeOutQuad),
                        parent: widget.controller,
                      ));
